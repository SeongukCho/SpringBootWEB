...
public class MailService implements IMailService {
    ...
    public int doSendMail(MailDTO pDTO) {
        ...
        // 전달 받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아
        // NULL이 발생할 수 있기 때문에 에러방지차원으로 if문 사용함
        if (pDTO == null) {
            pDTO = new MailDTO();
        }
        ...
        String contents = CmmUtil.nvl(pDTO.getContents()); // 메일
        ...
        // 로그 찍기(추후에 찍은 로그를 통해 이 함수 호출이 끝났는지 파악하기 용이하다.)
